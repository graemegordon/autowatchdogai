...
export async function generateATIPForm(data: ATIPFormData): Promise<Uint8Array> {
  ...
  drawText(page1, "Government of Canada / Gouvernement du Canada", margin + 10, height - 48, helvetica, 8, DARK_GRAY);
  drawText(page1, "TBS/SCT 350-57E", width - margin - 90, height - 35, helveticaBold, 9, DARK_GRAY);
  drawText(page1, "Rev. 2023-01", width - margin - 90, height - 48, helvetica, 8, MID_GRAY);

  drawBox(page1, margin, height - 105, contentWidth, 32, RED);
  drawText(page1, "ACCESS TO INFORMATION REQUEST / DEMANDE D'ACCÈS À L'INFORMATION", margin + 10, height - 87, helveticaBold, 9.5, WHITE);
  drawText(page1, "Access to Information Act / Loi sur l'accès à l'information", margin + 10, height - 100, helvetica, 8, WHITE);
  ...
  const institution = getInstitutionByName(data.institution) || Object.values(INSTITUTIONS)[0];
  ...
  const description = data.edited_description
    ? data.edited_description.replace(/<[^>]*>/g, "")
    : data.description;
  drawWrappedText(page1, description, margin + 5, y - 26, contentWidth - 12, helvetica, 9, 13, BLACK);
  ...
  page1.drawLine({ start: { x: margin + 5, y: y - 38 }, end: { x: margin + contentWidth * 0.65 - 10, y: y - 38 }, thickness: 0.5, color: BORDER });
  ...
  if (addressLines[1]) drawText(page1, addressLines[1], margin + 5, y - 34, helvetica, 8, BLACK);
  ...
  const steps = [
    { num: "1", title: "Complete the form", body: "Fill in all fields on page 1. Be as specific as possible in your description of records — vague requests may result in delays or missed documents." },
    { num: "2", title: "Sign the form", body: "Sign and date the form in Section 6. Unsigned requests will be returned." },
    { num: "3", title: "Prepare your $5.00 fee", body: `Write a cheque or money order for exactly $5.00 payable to:\n"Receiver General for Canada"\nDo NOT send cash. Personal cheques are accepted.` },
    { num: "4", title: "Mail to the institution", body: `Address your envelope to:\n${institution.name}\nATIP Coordinator\n${instAddress}${institution.phone ? `\n\nPhone: ${institution.phone}` : ""}` },
    { num: "5", title: "Track your request", body: "Keep a copy of this form. The institution must acknowledge your request within 30 days. Response time is up to 30 days (extensions possible). If you do not receive a response within 30 days, you can file a complaint with the Office of the Information Commissioner at 1-800-267-0441." },
  ];
  ...
  iy -= 75;
  drawWrappedText(page2, "The personal information collected on this form is used to process your request under the Access to Information Act (R.S.C. 1985, c. A-1). It is protected under the Privacy Act. Questions about this collection can be directed to the institution's ATIP Coordinator.", margin, iy, contentWidth, helveticaOblique, 7.5, 11, MID_GRAY);
